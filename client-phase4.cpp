...
void client(int numNeig,int* neigID, int* neigPORT,int myuid, string* myFiles,int numMyFiles,string myFilePath){
...
void server(int myid,int myuid,int MYPORT,int numNeig,string* fileName,int numFiles,string MYFILEPATH,string* myFiles,int numMyFiles){
    int listener;
    int newfd;
    int fdmax;
    int addrlen;
...
int main(int argc, char** argv){
    ...
    d=myFiles;
    fileDownloadDir+="/";
    thread t1(server, myid,myuid,MYPORT,numNeig,b,numFILES,fileDownloadDir,d,files.size());
    thread t2(client, numNeig,e,a,myuid,c,files.size(),MYFILEPATH);
    t1.join();
    t2.join();
